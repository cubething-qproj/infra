...
# Create root-level symlinks (idempotent -- `ln -sfn` replaces existing
# symlinks; a pre-existing real file/dir at the target will still cause ln to
# fail, which is the intended safety).
#
# Each entry is `src:dst`, where `src` is relative to `infra/main/` (the
# infra worktree root) and `dst` is relative to `$ROOT` (the metarepo
# root). The root `justfile` is intentionally not listed -- it is
# generated by the cp block below from `just/metarepo.just`.
ROOT_LINKS=(
	"metarepo/Cargo.toml:Cargo.toml"
	"metarepo/Cargo.lock:Cargo.lock"
	"metarepo/.zed:.zed"
	"config/config.toml:.cargo/config.toml"
...
for entry in "${ROOT_LINKS[@]}"; do
	...
	dst="${entry##*:}"
	link="$ROOT/$dst"
	target="infra/main/$src"
	if [ -e "$link" ] && [ ! -L "$link" ]; then
		echo "⚠  $dst exists as a real file/dir at metarepo root; skipping"
		echo "   (remove it manually if you want the symlink recreated)"
		continue
	fi
	mkdir -p "$(dirname "$link")"
	ln -srfn "$ROOT/$target" "$link"
	echo "✓ $dst → $target"
done
...
# customized).
agents_src="$SCRIPT_DIR/metarepo/AGENTS.md"
agents_dst="$ROOT/AGENTS.md"
if [ ! -e "$agents_dst" ]; then
	if [ ! -f "$agents_src" ]; then
		echo "✗ AGENTS.md template missing at $agents_src" >&2
		exit 1
	fi
	cp "$agents_src" "$agents_dst"
	echo "✓ AGENTS.md seeded from infra/main/metarepo/AGENTS.md"
else
	echo "✓ AGENTS.md exists (operator-owned; diff against"
	echo "   infra/main/metarepo/AGENTS.md to pick up template updates)"
fi
